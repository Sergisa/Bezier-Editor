...
public class Editor extends JPanel {
    ...
    private void renderString(Graphics graphics, FontMetrics metrics, String string, int lineNumber) {
        graphics.drawString(string, getWidth() - metrics.stringWidth(string) - 3, lineNumber * metrics.getHeight() + metrics.getAscent());
    }
    ...
    private MouseAdapter mouseListener = new MouseAdapter() {
        ...
        public void mouseReleased(MouseEvent e) {
            if (e.getButton() == MouseEvent.BUTTON1) {
                if (selectedPoint != null) bezier.save(CURRENT_CURVE_BACKUP_FILE);
                selectedPoint = null;
            }
            if (e.getButton() == MouseEvent.BUTTON2) isOffsetingViewport = false;
        }
    };
}
